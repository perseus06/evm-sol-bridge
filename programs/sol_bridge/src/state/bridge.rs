...
impl Bridge {
    ...
    pub fn update_balance(
        ...
        remote_chain_selector: u64, // EVM chain selector
        remote_token: String,       // Remote token address (on EVM)
        amount: u64,                // Amount to update
        flag: bool                  // true for add, false for subtract
    ) -> Result<()> {
        // Encode local_token as bytes
       let binding = local_token.to_string();
       let local_token_bytes = binding.as_bytes();
        ...
        // Find the index of the token ID in token_ids
        if let Some(index) = self.token_ids.iter().position(|id| id == &token_id) {
            // Update the balance based on the flag
            if flag {
                self.target_balances[index] = self.target_balances[index].checked_add(amount)
                    .ok_or(BridgeErrorCode::Overflow)?;
            } else {
                self.target_balances[index] = self.target_balances[index].checked_sub(amount)
                    .ok_or(BridgeErrorCode::Underflow)?;
            }
            Ok(())
        } else {
            Err(BridgeErrorCode::UnsupportedToken.into()) // Token ID or Chain Selector not found
